  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex space-x-4 border-b">
          <button onClick={() => setActiveTab('pending')} className={`px-4 py-2 font-medium flex items-center space-x-2 transition-all duration-200 active:scale-95 rounded-t-lg ${activeTab === 'pending' ? 'text-blue-600 bg-white border-b-2 border-blue-500 font-semibold shadow-sm' : 'text-slate-600 hover:text-blue-500 hover:bg-white/50'}`}>
            <Clock className="w-4 h-4" />
            <span>待接收 ({testTasks.filter(t => t.status === 'pending').length})</span>
          </button>
          <button onClick={() => setActiveTab('in-progress')} className={`px-4 py-2 font-medium flex items-center space-x-2 transition-all duration-200 active:scale-95 rounded-t-lg ${activeTab === 'in-progress' ? 'text-blue-600 bg-white border-b-2 border-blue-500 font-semibold shadow-sm' : 'text-slate-600 hover:text-blue-500 hover:bg-white/50'}`}>
            <AlertCircle className="w-4 h-4" />
            <span>检验中 ({testTasks.filter(t => t.status === 'in-progress').length})</span>
          </button>
          <button onClick={() => setActiveTab('completed')} className={`px-4 py-2 font-medium flex items-center space-x-2 transition-all duration-200 active:scale-95 rounded-t-lg ${activeTab === 'completed' ? 'text-blue-600 bg-white border-b-2 border-blue-500 font-semibold shadow-sm' : 'text-slate-600 hover:text-blue-500 hover:bg-white/50'}`}>
