...
export default function CountyDashboard(props) {
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full px-3 py-4 sm:px-4 lg:px-6">
        <div className="flex flex-col sm:flex-row sm:space-x-3 space-y-2 sm:space-y-0 border-b">
          <button onClick={() => setActiveTab('pending')} className={`px-2 sm:px-3 py-1.5 font-medium flex items-center space-x-1 sm:space-x-2 transition-all duration-200 active:scale-95 rounded-t-lg text-xs sm:text-sm ${activeTab === 'pending' ? 'text-blue-600 bg-white border-b-2 border-blue-500 font-semibold shadow-sm' : 'text-slate-600 hover:text-blue-500 hover:bg-white/50'}`}>
            <Clock className="w-4 h-4" />
            <span>待接收 ({testTasks.filter(t => t.status === 'pending').length})</span>
          </button>
          <button onClick={() => setActiveTab('in-progress')} className={`px-2 sm:px-3 py-1.5 font-medium flex items-center space-x-1 sm:space-x-2 transition-all duration-200 active:scale-95 rounded-t-lg text-xs sm:text-sm ${activeTab === 'in-progress' ? 'text-blue-600 bg-white border-b-2 border-blue-500 font-semibold shadow-sm' : 'text-slate-600 hover:text-blue-500 hover:bg-white/50'}`}>
            <AlertCircle className="w-4 h-4" />
            <span>检验中 ({testTasks.filter(t => t.status === 'in-progress').length})</span>
          </button>
          <button onClick={() => setActiveTab('completed')} className={`px-2 sm:px-3 py-1.5 font-medium flex items-center space-x-1 sm:space-x-2 transition-all duration-200 active:scale-95 rounded-t-lg text-xs sm:text-sm ${activeTab === 'completed' ? 'text-blue-600 bg-white border-b-2 border-blue-500 font-semibold shadow-sm' : 'text-slate-600 hover:text-blue-500 hover:bg-white/50'}`}>
          ...
          {filteredTasks.map(task => <Card key={task.id} className="hover:shadow-lg active:shadow-md active:scale-98 transition-all duration-200 cursor-pointer">
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start space-y-2 sm:space-y-0">
                  ...
                  <div className="flex flex-col space-y-1 sm:ml-3 mt-2 sm:mt-0">
